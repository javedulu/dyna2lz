...
// 1. Added option -- keep deleted elements. If on, field 'Deleted' will be
// created.
// 2. Minor fixes
//
...
// 3. SPH, RigidBody, user materials and all other non-generic things are
// skipped.
//
...
private:
  ...
  bool _accelerations;
  unsigned int
      _num_global_vars; // number of global variables to read each state
...
public:
  ...
  unsigned int dimensions() const { return _dimensions; };
  const char *model_descr() const { return _model_descr; };
  unsigned int runtime() const { return _runtime; };
  unsigned int rundate() const { return _rundate; };
  unsigned int machine() const { return _machine; };
  unsigned int codeid() const { return _codeid; };
  float codever() const { return _codever; };
  bool elem_conns() const { return _elem_conns; };
  bool mattypes() const { return _mattypes; };
  bool road_movement() const { return _road_movement; };
  unsigned int nodes() const { return _nodes; };
  bool code_is_new() const { return _code_is_new; };
  bool temperatures() const { return _temperatures; };
  bool cur_geom() const { return _cur_geom; };
  bool velocities() const { return _velocities; };
  bool accelerations() const { return _accelerations; };
  unsigned int num_global_vars() const { return _num_global_vars; };
  unsigned int num_8_node_elems() const { return _num_8_node_elems; };
  unsigned int num_8_node_mats() const { return _num_8_node_mats; };
  unsigned int num_8_node_vals() const { return _num_8_node_vals; };
  unsigned int num_8_node_add() const { return _num_8_node_add; };
  unsigned int num_2_node_elems() const { return _num_2_node_elems; };
  unsigned int num_2_node_mats() const { return _num_2_node_mats; };
  unsigned int num_2_node_vals() const { return _num_2_node_vals; };
  unsigned int num_4_node_elems() const { return _num_4_node_elems; };
  unsigned int num_4_node_mats() const { return _num_4_node_mats; };
  unsigned int num_4_node_vals() const { return _num_4_node_vals; };
  unsigned int num_4_node_add() const { return _num_4_node_add; };
  unsigned int num_4_node_int() const { return _num_4_node_int; };
  unsigned int elems_deletion() const { return _elems_deletion; };
  unsigned int sph_nodes() const { return _sph_nodes; };
  unsigned int sph_mats() const { return _sph_mats; };
  unsigned int narbs() const { return _narbs; };
  unsigned int thick_shell_elems() const { return _thick_shell_elems; };
  unsigned int thick_shell_mats() const { return _thick_shell_mats; };
  unsigned int thick_shell_vals() const { return _thick_shell_vals; };
  bool stress_components() const { return _stress_components; };
  bool plastic_strain() const { return _plastic_strain; };
  bool shell_force_res() const { return _shell_force_res; };
  bool shell_thickness_energy() const { return _shell_thickness_energy; };
  unsigned int fluid_mats() const { return _fluid_mats; };
  unsigned int cfd_nodal_flags1() const { return _cfd_nodal_flags1; };
  unsigned int cfd_nodal_flags2() const { return _cfd_nodal_flags2; };

  unsigned int total_cells() const {
    return _num_8_node_elems + _thick_shell_elems + _num_2_node_elems +
           _num_4_node_elems;
  };
...
typedef struct { float x, y, z; } node_coord_t;

typedef struct { float val[3]; } vector_3_t;

typedef struct { float val[2]; } vector_2_t;
...
private:
  ...
  unsigned int _elemKind;

public:
  GenericCell(unsigned int *nodes, unsigned char pts, unsigned int partID,
              unsigned int elemKind);
  ~GenericCell();

  unsigned char nodesCount() const { return _pts; };

  unsigned int partID() const { return _partID; };

  unsigned int node(unsigned char index) const { return _nodes[index]; };

  unsigned int elemKind() const { return _elemKind; };
};
...
typedef struct { float val[6]; } tensor_t;
...
private:
  ...
  // statistics
  unsigned int _points, _hexas, _lines, _triangles, _quads, _pyramids, _tetras,
      _wedges;
...
public:
  ...
  unsigned int getPointsCount() const { return _points; };
  unsigned int getHexasCount() const { return _hexas; };
  unsigned int getLinesCount() const { return _lines; };
  unsigned int getTrianglesCount() const { return _triangles; };
  unsigned int getQuadsCount() const { return _quads; };
  unsigned int getPyramidsCount() const { return _pyramids; };
  unsigned int getTetrasCount() const { return _tetras; };
  unsigned int getWedgesCount() const { return _wedges; };
  ...
  void setSigma(int grid, unsigned int localCell, float *val,
                shell_pos_t shellPos = shellMiddle);
  void setPlStrain(int grid, unsigned int localCell, float val,
                   shell_pos_t shellPos = shellMiddle);
  void setStrain(int grid, unsigned int localCell, float *val,
                 shell_pos_t shellPos = shellMiddle);
...
public:
  D3PlotState(StateOptions *opts, D3PlotControl *ctl, D3PlotGeometry *geo,
              D3PlotFile *f);
  ~D3PlotState();
